...
export default function HelperInvitationsList() {
  ...
  const fetchInvitations = async () => {
    try {
      ...
      if (response.ok) {
        if (data.invitations) {
          // Sort invitations by createdAt and filter to get the latest from each patient
          const latestInvitations = data.invitations
            ...
            .filter((invitation: Invitation, index: number, self: Invitation[]) =>
              index === self.findIndex((i: Invitation) => i.patientId === invitation.patientId)
            );
  ...
  if (invitations.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold mb-4">Patient Invitations</h2>
  ...
  return (
    <div className="bg-white rounded-lg shadow-md">
      <h2 className="text-xl font-semibold p-4 border-b">Patient Invitations</h2>
      
      <div className="divide-y divide-gray-200">
        {invitations.map((invitation) => (
          <div key={`${invitation.id}-${invitation.createdAt}`} className="p-4">
            <div className="mb-3">
              <div className="font-medium">{invitation.patientName}</div>
